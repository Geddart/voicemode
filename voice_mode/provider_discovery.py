...
This module handles automatic discovery of TTS endpoints, including:
- Health checks
- Model discovery
- Voice discovery
...
def is_local_provider(base_url: str) -> bool:
    ...
    return provider_type in ["kokoro", "local"] or \
...
class ProviderRegistry:
    ...
    async def initialize(self):
        ...
        async with self._discovery_lock:
            ...
            logger.info(f"Provider registry initialized with {len(self.registry['tts'])} TTS endpoints")

    async def _discover_endpoints(self, base_urls: List[str]):
        """Discover all TTS endpoints."""
        tasks = []
        for url in base_urls:
            if url not in self.registry["tts"]:
                tasks.append(self._discover_endpoint(url))

        if tasks:
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for url, result in zip(base_urls, results):
                if isinstance(result, Exception):
                    logger.error(f"Failed to discover TTS endpoint {url}: {result}")
                    self.registry["tts"][url] = EndpointInfo(
                        base_url=url,
                        models=[],
                        voices=[],
    ...
    async def _discover_endpoint(self, base_url: str) -> None:
        """Discover capabilities of a single TTS endpoint."""
        logger.debug(f"Discovering TTS endpoint: {base_url}")
        ...
        try:
            ...
            # Store endpoint info
            self.registry["tts"][base_url] = EndpointInfo(
                base_url=base_url,
                models=models,
                voices=voices,
                provider_type=detect_provider_type(base_url),
            ...
            logger.info(f"Successfully discovered TTS endpoint {base_url} with {len(models)} models and {len(voices)} voices")
    ...
    def get_endpoints(self) -> List[EndpointInfo]:
        """Get all TTS endpoints in priority order."""
        endpoints = []

        # Return endpoints in the order they were configured
        for url in TTS_BASE_URLS:
            info = self.registry["tts"].get(url)
            if info:
    ...
    def find_endpoint_with_model(self, model: str) -> Optional[EndpointInfo]:
        """Find the first TTS endpoint that supports a specific model."""
        for endpoint in self.get_endpoints():
    ...
    async def mark_failed(self, base_url: str, error: str):
        ...
        """
        if base_url in self.registry["tts"]:
            # Update error and last check time for diagnostics
            self.registry["tts"][base_url].last_error = error
            self.registry["tts"][base_url].last_check = datetime.now(timezone.utc).isoformat()
            logger.info(f"TTS endpoint {base_url} failed: {error}")
